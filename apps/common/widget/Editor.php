...
class Editor extends Base
{
    ...
    public function nkeditor($attributes = [])
    {
        if (!isset($attributes['id'])) $attributes['id'] = 'myeditor';//ID标识
        if (!isset($attributes['name'])) $attributes['name'] = 'content';//name值
        if (!isset($attributes['is_load_script'])) $attributes['is_load_script'] = 1;//是否加载脚本
        if (!isset($attributes['picturesModal'])) $attributes['picturesModal'] = 1;//是否显示多图按钮
        if (!isset($attributes['width'])) $attributes['width'] = '70%';//编辑器宽度
        if (!isset($attributes['height'])) $attributes['height'] = '200px';//编辑器高度
        //上传配置
        if (!isset($attributes['upload']['path_type'])) {
            $attributes['upload']['path_type'] = 'picture';
        ...
        //图片选择器弹窗地址
        if (!isset($attributes['pictures_dialog']['url'])) {
            $attributes['pictures_dialog']['url'] = url('admin/Upload/attachmentLayer', ['input_id_name' => $attributes['id'], 'path_type' => $attributes['upload']['path_type'], 'gettype' => 'multiple', 'from' => 'nkeditor']);
        }
         ...
         */
        $this->assign($attributes);
        return $this->fetch('common@widget/nkeditor');
    }
}
