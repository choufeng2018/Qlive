...
//                'live_type' => $top_living['live_room_info']['live_type'],
        ...
        $immediate_list = Db::name('QliveLiveHistory')
            ...
            ->where('flag', 'neq', 2)
            ->order('open_time')
            ->field('id as live_id,title,open_time,logo,anchor,category,live_type,description,flag,price')
            ->limit(3)
            ->select();
        ...
        $recommend_list = Db::name('QliveVideoList')
            ...
            ->order('order,update_time desc')
            ->field('id as video_id,title,anchor,live_id,category,logo,type,price,live_time,description')
            ->limit(3)
            ->select();
        ...
        //往期热门
        $hot_list = Db::name('QliveLiveHistory')
            ->where('status', 'eq', 1)
            ->order('hits desc')
            ->field('id as live_id,title,open_time,logo,anchor,category,live_type,description,flag,price')
            ->limit(6)
